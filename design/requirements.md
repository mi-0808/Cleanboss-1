...
- カメラ（タブレット/固定カメラ）が画像を取得
- AI推論により不備の有無を判定
- 不備があれば画面と音で警告、再チェックを促す
- 合格なら「入室OK」を表示（必要に応じてログ保存）
...
### 2.3 非機能要件（重要）
- **速度**：判定は数秒以内（目標：1〜3秒）
- **精度**：現場運用できるレベル（誤検知/見逃し許容の定義が必要）
- **操作性**：作業者が直感で分かる（日本語、色、音、やり直し導線）
- **安全性**：個人情報・顔の扱い（保存方針、マスキング、同意）
- **環境耐性**：照明条件、反射、画角、背景、ウェア色の影響を想定
- **拡張性**：チェック項目追加（靴・マスク・リストバンド等）に耐える設計
...
- 利用端末：タブレット（iPad/Android）か、固定PC＋カメラか
- カメラ配置：固定（スタンド/壁）か、手持ちか
- 判定対象：全身1枚で判定 or 上半身/手元の複数ショットが必要か
- 対象ウェア：形状（フード/チャック/ボタン位置）、色、サイズ差
- しきい値：どの程度の露出・隙間をNGとするか（運用ルール）
- データ保存方針：画像を保存するか、結果だけ保存するか
- 現場導線：入室ゲートと連動するか（OKで扉解錠、など）
...
### 5.1 運用・環境
1) 設置場所は「更衣室内（クリーンルーム前）」で確定ですか？（はい/いいえ）
2) 1回のチェックは「立って正面を向く」運用でOKですか？（OK/別の姿勢が必要）
3) 1人あたり許容チェック時間は何秒くらいですか？（例：5秒/10秒/20秒）
4) 照明は一定ですか？（一定/時間帯で変動/場所で差が大きい）
...
### 5.3 判定仕様（重要）
8) 4項目の優先度はありますか？（例：髪＞手袋＞チャック＞ボタン）
9) 「手袋の隙間」はどの状態をNGにしますか？（袖が見えたらNG / 肌が見えたらNG / 運用で決める）
10) 判定は「OK/NGの二値」でよいですか？（二値/注意・警告の3段階）

### 5.4 データ・セキュリティ
11) 判定ログは保存しますか？（保存しない / 結果のみ / 画像も保存）
12) 顔が写る場合、保存禁止ですか？（禁止/可/マスキングなら可）
13) 利用者識別は必要ですか？（不要/社員証IC等で紐づけたい/未定）
